...
static int init_ffmpeg()
{
    ...
    for (int i = 0; i < g_avformatctx->nb_streams; i++)
    {
        ...
        {
            g_vindex = i;
            const AVCodec* vcodec = avcodec_find_decoder(avstream->codecpar->codec_id);
        ...
        {
            g_aindex = i;
            const AVCodec* acodec = avcodec_find_decoder(avstream->codecpar->codec_id);
...
int SDLPlayer()
{
    // const char* file = "D:/Project/FFmpeg/file/Iron_Man-Trailer_HD.mp4";
    //strcpy(g_videopath, "./file/ba.mp4");
    strcpy(g_videopath, "D:/Project/FFmpeg/file/Iron_Man-Trailer_HD.mp4");
    on_init();
    int ret = main_loop();
    on_cleanup();
    return ret;
