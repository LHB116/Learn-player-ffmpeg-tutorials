...
enum
{
    AV_SYNC_AUDIO_MASTER, // Sync to audio clock.
    AV_SYNC_VIDEO_MASTER,  // Sync to video clock.
    AV_SYNC_EXTERNAL_MASTER,  // Sync to external clock: the computer clock
};
    ...
    for (;;)
    {
        ...
        if (videoState->seek_req)
        {
            ...
            if (video_stream_index >= 0 && audio_stream_index >= 0)
            {
                ...
                //a.den = AV_TIME_BASE;
                // AV_TIME_BASE_Q
                seek_target_video = av_rescale_q(seek_target_video, a, pFormatCtx->streams[video_stream_index]->time_base);
                seek_target_audio = av_rescale_q(seek_target_audio, a, pFormatCtx->streams[audio_stream_index]->time_base);
            }
...
int Tutorial07()
{
    ...
    const char* filename = "D:/Project/FFmpeg/file/11.mp4";
