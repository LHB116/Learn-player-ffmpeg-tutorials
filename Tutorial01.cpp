...
void saveFrame(AVFrame* avFrame, int width, int height, int frameIndex)
{
    ...
    if (ppm_format) {
         ...
         */
         ...
         // Open file
        sprintf(szFilename, "./output/T1_frame%d.ppm", frameIndex);
    ...
    //uint8_t* bgr_data[4];
    //int bgr_linesize[4];
    //av_image_alloc(bgr_data, bgr_linesize, width, height, AV_PIX_FMT_BGR24, 1);
    cv::Mat mat(height, width, CV_8UC3);
    mat.data = avFrame->data[0];
    sprintf(szFilename, "./output/T1_frame%d.png", frameIndex);
    cv::imwrite(szFilename, mat);
}
