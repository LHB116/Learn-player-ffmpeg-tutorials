...
static uint64_t in_channel_layout;
...
static enum AVSampleFormat forceFormat;
static SDL_AudioSpec wanted_specs;
...
static int audio_decode_frame0(AVCodecContext* aCodecCtx, uint8_t* audio_buf, int buf_size) {
...
static void audio_callback(void* userdata, Uint8* stream, int len)
{
    ...
    while (len > 0)
    {
        ...
        //SDL_MixAudioFormat(stream, (uint8_t*)audio_buf + audio_buf_index, AUDIO_S16SYS, len1, SDL_MIX_MAXVOLUME);
...
int Tutorial3()
{
    ...
    int ret = -1;
    const char* filename = "./file/ba.mp4";
    ...
    // open audio device
    SDL_OpenAudio(&wanted_specs, NULL);
    ...
    SDL_PauseAudio(0);   // [2]
    //SDL_PauseAudioDevice(0, audioDeviceID);   // [2]
    ...
    swr_alloc_set_opts(audio_s_ctx, out_channel_layout, AV_SAMPLE_FMT_S16, aCodecCtx->sample_rate,
    ...
    //SDL_Thread* thread1 = SDL_CreateThread(audio_play_thread, "audio_decode", aCodecCtx);
    //if (!thread1)
    //{
    ...
    while (av_read_frame(pFormatCtx, pPacket) >= 0)
    {
        ...
        else if (pPacket->stream_index == audioStream)
        {
            ...
            //printf("audioStream.\n");
            //printf("stream_index = %d, pPacket->pos= %d, data0 = %d, data1 = %d \n", pPacket->stream_index, pPacket->pos, pPacket->data[0], pPacket->data[1]);
