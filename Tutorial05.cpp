...
#define AV_SYNC_THRESHOLD 0.01   // 时钟差异小于该值不需要进行同步
#define AV_NOSYNC_THRESHOLD 1.0  // 时钟差异大于该值需要进行同步
...
static void video_refresh_timer(void* userdata)
{
    ...
    double real_delay;
    double audio_video_delay;  // 视频相较于音频时延
    ...
    if (videoState->video_st)
    {
        ...
        else
        {
            ...
            {
                if (audio_video_delay <= -sync_threshold) // 视频信号滞后，马上刷新视频
                {
                    pts_delay = 0;
                }
                else if (audio_video_delay >= sync_threshold)  // 视频信号超前了，先不刷新视频，等待音频同步上来
                {
...
static int stream_component_open(VideoState* videoState, int stream_index)
{
    ...
    if (codecCtx->codec_type == AVMEDIA_TYPE_AUDIO)
    {
        ...
        if (SDL_OpenAudio(&wanted_specs, NULL) < 0)
        {
...
static int decode_thread(void* arg)
{
    ...
    for (;;)
    {
        ...
        if (ret < 0)
        {
            ...
            {
                break; // exit for loop in case of error
            }
        }
...
int Tutorial05()
{
    const char* filename = "D:/Project/FFmpeg/file/Iron_Man-Trailer_HD.mp4";
    ...
    //char * pEnd;
    //videoState->maxFramesToDecode = strtol(filename, &pEnd, 10);
    videoState->maxFramesToDecode = 1000;
    ...
    for(;;)
    {
        ...
        switch(event.type)
        {
            ...
            case FF_REFRESH_EVENT:  // 延时push event
            {
                video_refresh_timer(event.user.data1); // 根据计算的延时播放视频
            }
            break;
